            ...
            <table className='text-[7px]  w-[100%] border-[1px] mt-1'>
              ...
              <section className='flex w-[100%] mt-2'>
                <div className='w-[30%] border-e-2 pe-3'>
                  ...
                  <p className='w-[95%] overflow-x-hidden text-[7px] noScrollBar mt-1' ><span className='text-blue-400 py-2'><span> <BiLogoGmail className='text-[13px] inline' /></span> :</span> {state.email}</p>
                  <p className='w-[95%] overflow-x-hidden noScrollBar'> <span className='text-blue-400 py-2'><FaPhoneAlt className='text-[12px] inline' /> : </span> {state.phoneNo}</p>
                  <p className='w-[95%] overflow-x-hidden noScrollBar'> <span className='text-blue-400 py-2'><MdHome className='text-[13px] inline' /> : </span> {state.address}</p>
                  <article>
                    ...
                    <div key={index}>
                      ...
                      <article>
                        ...
                        {state.experiences.map((experience, index) => (
                          <div key={index}>
                            <p>
                              <span className="text-black-400">Company: </span>
                              {experience.companyName}
                            </p>
                            <p>
                              <span className="text-black-400">Experience Duration: </span>
                              {experience.experienceStartDate} to {experience.experienceEndDate}
                            </p>
                            <div>
                              <span className="text-black-400">Key responsibilities: </span>
                              {experience.keyResponsibility.map((key, index) => (
                                <Fragment key={index}>
                                  <span>{key}, </span>
                                </Fragment>
                              ))}
